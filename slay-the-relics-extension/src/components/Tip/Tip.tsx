...
export type HitBox = { x: string; y: string; w: string; h: string; z: number | string; };
...
export function PowerTip(props: { tip: Tip, offset: { x: string, y: string } }) {
    const headerImgClass = props.tip.img === null ? "powertip-header-noimg" : "powertip-header-wimg";
...
export function PowerTipBlock(props: {
    ...
    return (
        <Hitbox magGlass={props.magGlass} hitbox={props.hitbox}>
            {chunks.map((chunk, i) => {
                return (
                    <div key={"power-tip-block-chunk-" + i} className={"powertip-multicol-column"}>
                        {chunk.map((tip, j) => {
                            return (
                                <PowerTip key={"power-tip-block-item-" + i + "-" + j} tip={tip}
                                          offset={props.offset}/>
                            )
                        })}
                    </div>
    ...
    return (
        <Hitbox magGlass={props.magGlass} hitbox={props.hitbox}>
            {props.tips.map((tip, i) => {
                return (
                    <PowerTip key={"power-tip-strip-" + i} tip={tip}
                              offset={props.offset}/>
                )
            })}
        </Hitbox>
