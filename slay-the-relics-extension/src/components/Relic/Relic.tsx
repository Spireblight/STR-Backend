...
export class RelicProp {
    ...
    getTips(): Tip[] {
        return [new Tip(this.name, this.description, null)].concat(this.additionalTips);
    }
}
...
export function Relic(props: { relic: RelicProp, hitbox: HitBox }) {
    return (
        <PowerTipStrip magGlass={true} hitbox={props.hitbox} tips={props.relic.getTips()}
                       offset={{x: "70%", y: "-30%"}}/>
    )
}
    ...
    return (
        <div id={"relic-bar"}>
            {props.relics.map((relic, i) => {
                const hitbox = {
                    x: (RELIC_HITBOX_LEFT + i * RELIC_HITBOX_WIDTH + multiPage * RELIC_HITBOX_MULTIPAGE_OFFSET) + '%',
                    y: 6.111 + '%',
                    z: 1,
                    w: 3.75 + '%',
                    h: 8.666 + '%'
                ...
                return (
                    <Relic key={"relic-bar-" + i} hitbox={hitbox} relic={relic}/>
                )
            })}
        </div>
    )
